...
Business tool errors contain a safe message, HTTP-equivalent `status` and stable `code`; unexpected errors are masked as `INTERNAL_ERROR`. Authentication failures return an HTTP 401 Bearer challenge so the client can reconnect. Operations not permitted by the current user’s CRM role return 403-equivalent tool errors; inaccessible records return 404. Query or field validation fails without partially applying an update.
